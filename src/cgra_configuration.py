...
class CgraConfiguration:
    ...
    def create_alu_conf(self, id, op, alu_src, alu_delay):
        ...
        opcode_bits = format(int(bin(opcode)[2:], 2), '0%db' % op_width)
        ...
        sel_alu = []
        sel_alu_bits = bits(len(neighbors) + n)

        for alu in alu_src:
            if alu == 'istream':
                if not pe_is_input:
                    return False, 'PE %s does not have input stream.' % id
                sel = format(int(bin(0)[2:], 2), '0%db' % sel_alu_bits)
                sel_alu.append(sel)
            elif alu == 'acc':
                if not has_acc:
                    return False, 'PE %s does not accumulator.' % id
                sel = format(int(bin(1)[2:], 2), '0%db' % sel_alu_bits)
                sel_alu.append(sel)
            elif alu == 'const':
                sel = format(int(bin(n - 1)[2:], 2), '0%db' % sel_alu_bits)
                sel_alu.append(sel)
            else:
                try:
                    pe_src = neighbors.index(alu) + n
                    sel = format(int(bin(pe_src)[2:], 2), '0%db' % sel_alu_bits)
                    sel_alu.append(sel)
                except:
        ...
        if elastic_queue > 0:
            elastic_queue_latency = [format(int('0', 2), '0%db' % elastic_queue_latency_bits) for _ in
                                     range(alu_num_inputs)]
            if alu_delay is not None:
                for i, v in alu_delay:
                    elastic_queue_latency[i] = format(int(bin(v)[2:], 2), '0%db' % elastic_queue_latency_bits)

            elastic_queue_latency.reverse()
            elastic_queue_latency = ''.join(elastic_queue_latency)

        raw_conf = format(int(elastic_queue_latency + sel_alu + opcode_bits + '01' + id_bits, 2), '0%db' % conf_bits)
        return True, self.raw_conf_to_packtes(raw_conf, '0', '1')
    ...
    def create_const_conf(self, id, const):
        ...
        const = format(int(bin(const)[2:], 2), '0%db' % self.cgra.data_width)
        raw_conf = format(int(const + '10' + id_bits, 2), '0%db' % conf_bits)
        return True, self.raw_conf_to_packtes(raw_conf, '0', '1')
    ...
    def create_router_conf(self, id, routing):
        ...
        if routing is not None and pe_has_routing:
            map_route = {}
            ...
            return False, 'PE %s cannot perform routing.' % id
        ...
        if routes > 0:
            if routes == 1:
                if len(routing.keys()) == 1:
                    for i, _ in routing.items():
                        if i == 'alu':
                            route_sel_in = format(int('0', 2), '0%db' % route_sel_in_bits)
                        else:
                            iidx = neighbors.index(i) + 1  # the first port is always alu
                            route_sel_in = format(int(bin(iidx)[2:], 2), '0%db' % route_sel_in_bits)
                ...
                if len(routing.keys()) > routes:
                    ...
                    if routes == len(neighbors):
                        route_sel_in_v = [format(int('0', 2), '0%db' % route_sel_in_bits) for _ in
                                          range(num_out)]
                        for i, o in routing.items():
                            if o == 'ostream':
                                oidx = len(neighbors)
                            ...
                            if i == 'alu':
                                route_sel_in_v[oidx] = format(int('0', 2), '0%db' % route_sel_in_bits)
                            else:
                                iidx = neighbors.index(i) + 1  # the first port is always alu
                                route_sel_in_v[oidx] = format(int(bin(iidx)[2:], 2), '0%db' % route_sel_in_bits)
                        route_sel_in_v.reverse()
                        route_sel_in = "".join(route_sel_in_v)
                    else:
                        route_sel_in_v = []
                        route_sel_out_v = [format(int('0', 2), '0%db' % route_sel_out_bits) for _ in range(num_out)]
                        for i, o in routing.items():
                            if o == 'ostream':
                                oidx = len(neighbors)
                            else:
                                oidx = neighbors.index(o)
                            if i == 'alu':
                                route_sel_in_v.append(format(int('0', 2), '0%db' % route_sel_in_bits))
                                route_sel_out_v[oidx] = format(int(bin(oidx)[2:], 2), '0%db' % route_sel_out_bits)
                            else:
                                iidx = neighbors.index(i) + 1
                                oidx = neighbors.index(o)
                                route_sel_in_v.append(format(int(bin(iidx)[2:], 2), '0%db' % route_sel_in_bits))
                                route_sel_out_v[oidx] = format(int(bin(oidx)[2:], 2), '0%db' % route_sel_out_bits)
                        route_sel_in_v.reverse()
        ...
        raw_conf = format(int(route_sel_out + route_sel_in + '11' + id_bits, 2), '0%db' % conf_bits)
        return True, self.raw_conf_to_packtes(raw_conf, '0', '1')
    ...
    def create_reset_conf(self, id):
        ...
        conf_bits = self.cgra.pe_id_width + max(self.cgra.data_width, pe_conf_bits)
        id_bits = format(int(bin(id + 1)[2:], 2), '0%db' % self.cgra.pe_id_width)
        raw_conf = format(int('00' + id_bits, 2), '0%db' % conf_bits)
        return True, self.raw_conf_to_packtes(raw_conf, '0', '1')

    def raw_conf_to_packtes(self, raw_conf, start_packet, end_packet):
        packet = []
